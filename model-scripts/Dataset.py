...
class Dataset:
    ...
    def load_house_dataframe(self, house):
        df = pd.read_csv('../REDD/houses/house_' + str(house) + '.csv')
    ...
    def format_for_keras(
        ...
        X, Y = resample(mains_array, appliance_array, n_samples,random_state)
    ...
    def create_sliding_windows(self, array):
        timesteps = len(array)
        part_1 = np.arange(self.window_size)[None,:]
        part_2 = np.arange(timesteps - self.window_size + 1)[:, None]
...
d = Dataset()
for i in range(2, 7):
    d.load_house_dataframe(i)
    d.add_mains_col(i)
    d.add_timestep_col(i)
    d.dataframes[i].to_csv('house_' + str(i) + '.csv')

#d.add_windows(1, '05 refrigerator')
